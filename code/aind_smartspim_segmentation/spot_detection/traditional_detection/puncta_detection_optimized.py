...
def get_background_percentage(data):

    smoothed_image = ndi.gaussian_filter(data, sigma=1.5)
    thresh = threshold_otsu(smoothed_image)
    binary = smoothed_image <= thresh

    total_pixels = np.prod(smoothed_image.shape)
    
    non_zero = np.count_nonzero(binary)
    
    background_percentage = (non_zero / total_pixels) * 100
...
def traditional_3D_spot_detection(
    ...
    background_percentage = np.rint(100 - get_background_percentage(data_block)).astype(np.uint8)
